...
vin_app = tk.Tk()
vin_app.geometry('900x600')
vin_app.title('Vin Text Editor')
vin_app.wm_iconbitmap('icon.ico')
...
text_editor = tk.Text(vin_app)
text_editor.config(wrap='word', relief=tk.FLAT)

scroll_bar = tk.Scrollbar(vin_app)
...
# font family & font size functions
current_font_family = 'Arial'
current_font_size = 12

def change_font(vin_app):   # event=None
    global current_font_family
    current_font_family = font_family.get()
    text_editor.configure(font=(current_font_family, current_font_size))

def change_fontsize(vin_app):
...
# status bar
status_bar = ttk.Label(vin_app, text='Status Bar')
...
def changed(event=None):
    ...
    if text_editor.edit_modified():
        ...
        characters = len(text_editor.get(1.0, 'end-1c')) #.replace(' ',''))   add this line to avoid space count
...
def open_file(event=None):
    ...
    try:
        ...
        return
    vin_app.title(os.path.basename(url))
...
view.add_checkbutton(label='Tool Bar',onvalue=True, variable = show_toolbar, offvalue=0, image=tool_bar_icon, compound=tk.LEFT, command=hide_toolbar)
...
vin_app.config(menu=main_menu)
vin_app.bind('<Control-n>', new_file)
vin_app.bind('<Control-o>', open_file)
vin_app.bind('<Control-s>', save_file)
vin_app.bind('<Control-Alt-s>', save_as)
vin_app.bind('<Control-q>', exit_func)
vin_app.bind('<Control-f>', find_func)
